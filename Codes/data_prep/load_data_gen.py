...
class GenDataset(torch.utils.data.Dataset):
    def __init__(self, config, mode):
        ...
        self.files = lines
        filenames_ = self.check_images(config)
        self.files = filenames_
        self.transform = transforms.Compose([
                            transforms.ToPILImage(),
                            transforms.Resize(64),
                            transforms.ToTensor()])

    def check_images(self, config):
        if not config.data.apply_filter: raise NotImplementedError
        folder = os.path.join(config.generative_config.path, config.data.filter)
        if not os.path.exists(folder):
            os.makedirs(folder)
        filestems = set([Path(l).stem for l in self.files])
        folderstems = set([l.split('_')[0] for l in os.listdir(folder)])
        filenames =  [l for l in filestems if l not in folderstems]
        filenames_ = [os.path.join(folder, l) for l in os.listdir(folder) if l.split('_')[0] in filestems]
        if len(filenames) == 0: return filenames_
        for l in tqdm(self.files):
            
            data = torch.load(l)['images'].numpy()
            text = torch.load(l)['labelseq_id'].numpy()
            for idx in range(len(data)):
                with open(os.path.join(folder, Path(l).stem + '_' + str(idx)+'.npy'), 'wb') as f:
                    np.save(f, [data[idx], text[idx]])
        filenames_ = [os.path.join(folder, l) for l in os.listdir(folder) if l.split('_')[0] in filestems]
        return filenames_
    ...
    def __getitem__(self, i):
        with open(self.files[i], 'rb') as f:
            data, text = np.load(f, allow_pickle=True)
        
        return self.transform(data), torch.tensor(text)
